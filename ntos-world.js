...
function figFace(ctx, rig, cx, cy, r, mood, o){
  ...
  if(badger){                            // bandit mask + pale muzzle behind the eyes
    ...
    ctx.beginPath(); ctx.ellipse(cx, cy + r * 0.6, r * 0.54, r * 0.46, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#2a2520'; ctx.beginPath(); ctx.ellipse(cx, cy + r * 0.44, 1.2 * S, 0.9 * S, 0, 0, Math.PI * 2); ctx.fill();
  }
  // rosy cheeks — warmth
  ...
  // eyes: big, glossy, forward-looking
  [-1, 1].forEach(sgn => {
