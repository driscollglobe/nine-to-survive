...
function drawActor(ctx, cam, a, w){
  ...
  const dome = a.id === 'adam' ? 7.2 * z : 6.5 * z;   // Adam's is... more
  ctx.beginPath(); ctx.arc(px, py - 26 * z, dome, 0, Math.PI * 2);
  ctx.fillStyle = a.bear ? '#7d7871' : a.id === 'adam' ? '#EFD3AE' : '#E8C39E'; ctx.fill();
  ctx.strokeStyle = 'rgba(21,18,13,0.3)'; ctx.lineWidth = 1.1 * z; ctx.stroke();
  // key/fill rim: the 5pm sun catches the upper-left of everyone; a cool screen-
  // glow answers on the lower-right — the double key-light, on every silhouette
  const RL = WT.LIGHT;
  if(RL && z > 0.62){
    ctx.save(); ctx.lineCap = 'round';
    ctx.strokeStyle = hexA(RL.rimWarm, 0.5); ctx.lineWidth = 1.5 * z;
    ctx.beginPath(); ctx.ellipse(px, py - 11 * z, 8 * z, 11 * z, 0, Math.PI * 0.78, Math.PI * 1.5); ctx.stroke();
    ctx.beginPath(); ctx.arc(px, py - 26 * z, dome, Math.PI * 0.85, Math.PI * 1.55); ctx.stroke();
    ctx.strokeStyle = hexA(RL.rimCold, 0.34); ctx.lineWidth = 1.3 * z;
    ctx.beginPath(); ctx.ellipse(px, py - 11 * z, 8 * z, 11 * z, 0, Math.PI * -0.22, Math.PI * 0.34); ctx.stroke();
    ctx.restore();
  }
  if(a.id === 'adam'){
    // the shine: readable from across the floor
    ctx.beginPath(); ctx.ellipse(px - 2.2 * z, py - 29.5 * z, 2.6 * z, 1.5 * z, -0.5, 0, Math.PI * 2);
    ctx.fillStyle = 'rgba(255,255,255,0.85)'; ctx.fill();
  }
  // the interception, visible: he is talking AT you
  if(a.id === 'adam' && w && w.intercept){
    ...
    ctx.font = '700 ' + Math.max(8, 9 * z) + 'px Poppins, sans-serif';
    ctx.fillText('“quick thought—”', px, py - 44 * z);
  }
  if(a.bear){
