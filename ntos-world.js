...
const RIG = {
  you:   { h:0.92,w:2.05,shoulder:1.45,hip:1.95, legLen:0.40,torso:1.16,headScale:1.42, posture:'shrug',
...
function drawFigure(ctx, px, py, z, rig, o){
  ...
  if(rig.badger){
    ...
    // OBESE: the body is a near-sphere that swallows the legs and half the neck.
    // Wider than tall at the equator, sagging low over the stub feet.
    const midY = (shY + hipY) / 2 + 2.6 * S;
    const rx = hipW * 0.92, ry = (hipY - shY) * 0.85 + 4.5 * S;
    ...
    if(!sil){
      ctx.beginPath(); ctx.ellipse(topX * 0.5, midY + ry * 0.14, rx * 0.68, ry * 0.72, 0, 0, Math.PI * 2);
      ctx.fillStyle = '#d9d2c4'; ctx.fill();                       // the enormous tummy
    }
  } else {
    ctx.beginPath();
